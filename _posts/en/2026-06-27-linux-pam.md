...
I am a software engineer and have been using Linux for my personal and professional computers' operating system. I might be also a little obsessed with security and prefer strong passwords. Of course, modern password managers solve this problem for web browsers but I need to make the authentication process easier for sudo password. I decided to go with my laptop's webcam which also provides IR band images that would be very useful for liveness check for face detection. So, I decided to use my webcam to authorize applications for sudo password. Of course, there are very limited number of ready-to-shelf solutions like Howdy, and Biopass but I like learning. practicing, and developing new stuff. So, I decided to implement my solution that:
- Uses deep learning,
- Together with old/classic vision based approaches,
- Check liveness by using IR feed,
- and ensembling and deciding if the face is in application database.

In this article, I will explain how Linux can handle/help such a use case and not naive but definitely needs some attention solution for face recognition/identification. I didn't train any deep learning model instead used a yolo based face model. Let's first start with system side of the problem.

# Pluggable Authentication Modules
>PAM is a system of libraries that handle the authentication tasks of applications (services) on the system. The library provides a stable general interface (Application Programming Interface - API) that privilege granting programs defer to to perform standard authentication tasks[^1]. 
...
The authetication using PAM consists three layers[^3]:
...
Each configuration file (Layer-2) for PAM-aware applications contains a group of directives define the module and any controls or arguments with it[^4],[^5]. 
...
My plan is to first try to authenticate via my custom shared object, in case of a failure or unavailablity, fallback to `pam_unix.so` via `common-auth`:
...
Notice the module ou never returns `PAM_IGNORE`. This means every failure path is a hard authentication failure, which is the right behaviour when combined with sufficient in the PAM config. PAM will fall through to common-auth only on `PAM_AUTH_ERR`, not silently.

---

That covers the system side of the problem. The PAM layer gives us a clean hook — `pam_sm_authenticate` — where we can plug in any authentication logic without touching the applications that use it. To summarise what we've done on this side:
